...
fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    if os_args.len() == 2 && (os_args[1] == "-V" || os_args[1] == "--version") {
        let version = colorful_version_full!("sendgrowl", env!("CARGO_PKG_VERSION"), env!("CARGO_PKG_AUTHORS"));
    ...
    if args.version {
        colorful_version_full!("sendgrowl", env!("CARGO_PKG_VERSION"), env!("CARGO_PKG_AUTHORS")).print_and_exit();
    }
